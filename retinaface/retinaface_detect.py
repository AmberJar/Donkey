...
from retinaface.utils.general import draw_detections
...
def parse_arguments():
    ...
    parser.add_argument(
        '-w', '--weights',
        type=str,
        default='./retinaface/weights/final.pth',
...
@torch.no_grad()
def inference_landmarks(model, image):
    model.eval()
...
def get_landmarks(params):
    ...
    # forward pass
    loc, conf, landmarks = inference_landmarks(model, image) # location, conf, landmarks
    ...
    best_landmarks = best_landmarks.reshape(5, 2).astype(int) # 左眼右边鼻子左嘴右嘴
