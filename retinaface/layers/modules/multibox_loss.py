...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, ground_truth):
        ...
        # Optional: Calculate batch NME for logging
        with torch.no_grad():
            pred_landm = landmark_preds[pos_idx1].view(-1, 5, 2)
            gt_landm = lnm_targets.view(-1, 5, 2)

            inter_ocular = torch.norm(gt_landm[:, 0] - gt_landm[:, 1], dim=1)  # left-eye vs right-eye
            point_error = torch.norm(pred_landm - gt_landm, dim=2).mean(dim=1)  # per-sample avg error
            nme_batch = (point_error / inter_ocular).mean().item()  # mean over batch
        ...
        return loc_loss, conf_loss, loss_landm,nme_batch
