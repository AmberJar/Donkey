...
def test_feature_library(images_dir, txt_dir, selected_list, library_path, device="cuda"):
    ...
    feature_lib = torch.tensor(lib["features"], dtype=torch.float32).to(device)  # shape: [N, D]
    label_lib = lib["labels"]  # shape: [N]
    ...
    for img_name in tqdm(selected_set, desc="Testing feature library"):
        img_path = Path(images_dir) / img_name
        txt_path = Path(txt_dir) / Path(img_name).with_suffix(".txt")

        img = cv2.imread(str(img_path))
        if img is None:
            continue

        bbox = read_yolo_bbox(str(txt_path), img.shape)
        if bbox is None:
            continue
        ...
        with torch.no_grad():
            ...
            feat = F.normalize(feat, p=2, dim=1)  # shape: [1, D]

        # 计算与库中所有向量的余弦相似度
        similarity = torch.matmul(feat, feature_lib.T)  # [1, N]
        top1_idx = torch.argmax(similarity, dim=1).item()
        pred_label = label_lib[top1_idx]
        gt_label = extract_label(img_name)
    ...
    print(f"[RESULT] Top-1 Accuracy on test set: {acc:.2f}% ({correct}/{total})")
...
if __name__ == "__main__":
    ...
    library_path = "./output_features/features_and_labels_face.npz"

    test_feature_library(images_dir, txt_dir, selected_list, library_path)
